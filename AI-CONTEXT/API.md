...
O POST aceita opcionalmente `idempotency_key` (até 36 caracteres), gerada pelo formulário para que retries de rede do mesmo lançamento não criem duplicidade. A chave é única por `tenant_id` e deve ser renovada quando qualquer dado do lançamento mudar, inclusive `tipo_acesso` (`Entrada`/`Saída`); uma segunda tentativa com a mesma chave retorna `sucesso: true`, os dados do registro já existente e `idempotente: true`. Clientes antigos que não enviam a chave continuam funcionando sem deduplicação nova.

A ação `verificar_tag` de `api/api_rfid.php`, usada pela verificação manual de acesso no painel, também grava `usuario_liberou` e a ação `ultimos_acessos` devolve esse valor para a tela de detalhes. A listagem GET de `api/api_registros.php` também retorna `usuario_liberou` para a tabela de registros recentes. A ação `webhook` continua sem usuário, pois representa uma liberação automática recebida diretamente do equipamento RFID.

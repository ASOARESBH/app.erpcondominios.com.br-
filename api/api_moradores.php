...
$tenant_id = exigirTenantId();
    ...
    if ($metodo === 'GET' && isset($_GET['id'])) {
        ...
        try {
            $stmt = $conexao->prepare("SELECT id, nome, cpf, unidade, email, telefone, celular, ativo, observacao, DATE_FORMAT(data_cadastro, '%d/%m/%Y %H:%i') as data_cadastro FROM moradores WHERE tenant_id = $tenant_id AND id = ?");
            if (!$stmt) {
    ...
    if ($metodo === 'GET') {
        ...
        // Busca genérica: pesquisa em nome, CPF e unidade simultaneamente (OR)
        ...
        if ($filtro_busca) {
            // Busca genérica: nome, unidade (número ou "Gleba X") ou CPF
            $like_busca = "%" . $filtro_busca . "%";
            $cpf_busca  = preg_replace('/[^0-9]/', '', $filtro_busca);
            if ($cpf_busca !== '') {
                $where .= " AND (nome LIKE ? OR unidade LIKE ? OR REPLACE(REPLACE(REPLACE(cpf, '.', ''), '-', ''), ' ', '') LIKE ?)";
                $tipos_param .= "sss";
                $params[] = $like_busca;
                $params[] = $like_busca;
                $params[] = "%" . $cpf_busca . "%";
            } else {
                $where .= " AND (nome LIKE ? OR unidade LIKE ?)";
                $tipos_param .= "ss";
                $params[] = $like_busca;
                $params[] = $like_busca;
            }
    ...
    if ($metodo === 'PUT') {
        ...
        if (isset($dados['senha']) && !empty($dados['senha'])) {
            ...
            $stmt = $conexao->prepare("UPDATE moradores SET nome=?, cpf=?, unidade=?, email=?, telefone=?, celular=?, senha=?, observacao=? WHERE tenant_id = $tenant_id AND id=?");
            if (!$stmt) {
            ...
            $stmt = $conexao->prepare("UPDATE moradores SET nome=?, cpf=?, unidade=?, email=?, telefone=?, celular=?, observacao=? WHERE tenant_id = $tenant_id AND id=?");
            if (!$stmt) {
                throw new Exception("Erro ao preparar update: " . $conexao->error);
            }

...
proc: BEGIN
 ...
 FROM INFORMATION_SCHEMA.COLUMNS WHERE BINARY TABLE_SCHEMA=BINARY 'inlaud99_erpcondor' AND BINARY TABLE_NAME=BINARY p_tabela AND BINARY COLUMN_NAME=BINARY 'id' LIMIT 1;
 IF v_id_tipo IS NULL THEN
   LEAVE proc;
 END IF;
 SELECT COUNT(*) INTO v_pk FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE
 WHERE BINARY TABLE_SCHEMA=BINARY 'inlaud99_erpcondor' AND BINARY TABLE_NAME=BINARY p_tabela AND BINARY CONSTRAINT_NAME=BINARY 'PRIMARY' AND BINARY COLUMN_NAME=BINARY 'id';
